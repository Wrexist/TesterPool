...
insert into economy_config (key, value, note) values
  ...
  ('cost_buffer_seat',    145, 'One extra tester beyond the pod default. Roughly one full cycle of tester work.'),
  ...
  ('penalty_dropout',     120, 'Deducted when you abandon a pod mid-cycle.'),
...
language sql stable set search_path = public as $$ select value from economy_config where key = k $$;
...
begin
  select pods_completed, pods_dropped into v_completed, v_dropped from profiles where id = p_user;

  select coalesce(sum(days_checked_in),0),
         coalesce(sum(case when status in ('active','graduated','dropped') then 14 else 0 end),0)
    into v_days, v_expected from assignments where tester_id = p_user;

  select count(*) filter (where status = 'approved'), count(*) filter (where status = 'rejected')
    into v_feedback, v_rejected from feedback where tester_id = p_user;

  v_score := 70
    + least(15, v_completed * 3)
    + case when v_expected > 0 then (v_days::numeric / v_expected) * 15 else 0 end
    + least(10, v_feedback * 1.5)
    - v_dropped * 25
    - v_rejected * 4;
...
begin
  select a.tester_id, p.starts_at into v_tester, v_start
    from assignments a join pods p on p.id = a.pod_id where a.id = p_assignment;
...
begin
  select a.owner_id, f.tester_id, f.severity into v_owner, v_tester, v_sev
  ...
  if p_verdict = 'useful' then
    ...
    if v_bounty > 0 then
      perform award_credits(v_tester, v_bounty, 'bug_bounty', 'feedback', p_feedback, 'Blocker-severity bug with repro steps');
    end if;
    perform recompute_reliability(v_tester);
...
begin
  ...
  if p_uphold then
    ...
    update disputes set status = 'upheld', resolver_id = auth.uid(), resolution = p_resolution, resolved_at = now() where id = p_dispute;
  else
    v_pay := cfg('feedback_approved');
    ...
    update disputes set status = 'overturned', resolver_id = auth.uid(), resolution = p_resolution, resolved_at = now() where id = p_dispute;
  end if;
  perform recompute_reliability(v_tester);
...
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_owner uuid; v_pod uuid; v_count int; v_rel numeric; v_active int;
begin
  ...
  select p.id into v_pod from pods p
   ...
   order by (select count(*) from pod_members m where m.pod_id = p.id) desc limit 1;
  ...
  values (v_pod, v_owner, p_app, 'core', 'joined') on conflict (pod_id, user_id) do nothing;
...
begin
  ...
  perform award_credits(new.id, cfg('signup_grant'), 'signup_grant', 'profile', new.id, 'Welcome to Cohort');
...
create trigger assignments_no_self_test
  before insert or update on assignments for each row execute function guard_no_self_test();