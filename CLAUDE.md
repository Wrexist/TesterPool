...
app/                    Next.js 16 App Router application
  src/app/              routes — (app)/ is authenticated, everything else is public
  src/components/ui/    design primitives — StreakStrip, ReliabilityGauge, CreditChip…
  src/components/app/   authenticated-surface components
  src/lib/economy.ts    earn/spend rates, tiers, plans (mirrors economy_config table)
  src/lib/pods.ts       pod-day maths, seat health, formatting helpers
  src/lib/evidence.ts   drafts Google's three production-access answers from real data
  supabase/migrations/  applied in filename order
design/                 standalone design system + screen mockups (open in a browser)
docs/                   STRATEGY.md, BUILD-PLAN.md
