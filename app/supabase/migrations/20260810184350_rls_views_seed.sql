...
create policy "apps own" on apps for all using (owner_id = auth.uid()) with check (owner_id = auth.uid());
...
create policy "pod members own row" on pod_members for update using (user_id = auth.uid()) with check (user_id = auth.uid());

create policy "assignments visible to tester" on assignments for select using (tester_id = auth.uid());
...
create policy "proofs own" on proofs for all using (uploader_id = auth.uid()) with check (uploader_id = auth.uid());
create policy "proofs visible to app owner" on proofs for select
  using (exists (select 1 from assignments a join apps ap on ap.id = a.app_id
...
create policy "feedback own" on feedback for all using (tester_id = auth.uid()) with check (tester_id = auth.uid());
...
create policy "greenlights own" on greenlights for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "referrals own" on referrals for select using (referrer_id = auth.uid() or referee_id = auth.uid());
...
select p.id, p.handle, p.display_name, p.avatar_url, p.country_code,
       p.reliability, p.tier, p.pods_completed, p.apps_helped_ship, p.longest_streak,
...
create or replace view pod_health with (security_invoker = on) as
select pd.id, pd.code, pd.name, pd.status, pd.core_seats, pd.required_testers, pd.starts_at, pd.ends_at,
...
create or replace view production_evidence with (security_invoker = on) as
select ap.id as app_id, ap.owner_id, ap.name,
  ...
  min(pd.starts_at) as test_started, max(pd.ends_at) as test_ends
from apps ap
